...
abstract class Base_DB_ORM_Model extends Kohana_Object {
	...
	public function __get($name) {
		if ($this->is_field($name)) {
			...
			throw new Kohana_InvalidProperty_Exception('Thông báo: Không thể có được thuộc tính được chỉ định. Lý do: Thuộc tính: key không thể truy cập được hoặc không xác định được.', array(':key' => $name));
		}
	}
	...
	public function __set($name, $value) {
		if ($this->is_field($name)) {
			...
			throw new Kohana_InvalidProperty_Exception('Thông báo: Không thể đặt thuộc tính được chỉ định. Lý do : Thuộc tính key không thể truy cập được hoặc không xác định được.', array(':key' => $name, ':value' => $value));
		}
	}
	...
	public function delete($reset = FALSE) {
		...
		if ( ! $is_savable) {
			throw new Kohana_Marshalling_Exception('Thông báo: Không thể xóa bản ghi khỏi cơ sở dữ liệu. Lý do: Mô hình không thể lưu được.', array(':class' => self::get_called_class()));
		}
		$primary_key = call_user_func(array($self, 'primary_key'));
		if ( ! is_array($primary_key) || empty($primary_key)) {
			throw new Kohana_Marshalling_Exception('Thông báo: Không thể xóa bản ghi khỏi cơ sở dữ liệu. Lý do: Không có khóa chính nào được khai báo.');
		}
	...
	protected function hash_code() {
		...
		if (is_array($primary_key) && ! empty($primary_key)) {
			...
			foreach ($primary_key as $column) {
				if ( ! isset($this->fields[$column])) {
					throw new Kohana_InvalidProperty_Exception('Thông báo: Không thể tạo mã băm cho mô hình. Lý do: Khóa chính chứa tên trường không tồn tại.', array(':primary_key' => $primary_key));
				}
		...
		throw new Kohana_EmptyCollection_Exception('Thông báo: Không thể tạo mã băm cho mô hình. Lý do: Không có khóa chính nào được khai báo.', array(':primary_key' => $primary_key));
	}
	...
	public function load(Array $columns = array()) {
		if (empty($columns)) {
			...
			$primary_key = call_user_func(array($self, 'primary_key'));
			if ( ! is_array($primary_key) || empty($primary_key)) {
				throw new Kohana_Marshalling_Exception('Thông báo: Không thể tải bản ghi từ cơ sở dữ liệu. Lý do: Không có khóa chính nào được khai báo.');
			}
			$data_source = call_user_func(array($self, 'data_source'));
			$table = call_user_func(array($self, 'table'));
	...
	public function save($reload = FALSE) {
		$self = get_class($this);
        if($self!="Model_Leap_User")Helper_RDF_Store::indexModel($this);
        //if($self!="Model_Leap_User")die;
		$is_savable = call_user_func(array($self, 'is_savable'));
		if ( ! $is_savable) {
			throw new Kohana_Marshalling_Exception('Thông báo: Không lưu được bản ghi vào cơ sở dữ liệu. Lý do: Mô hình không thể lưu được.', array(':class' => self::get_called_class()));
		}
		$primary_key = call_user_func(array($self, 'primary_key'));
		if ( ! is_array($primary_key) || empty($primary_key)) {
			throw new Kohana_Marshalling_Exception('Thông báo: Không lưu được bản ghi vào cơ sở dữ liệu. Lý do: Không có khóa chính nào được khai báo.');
		}
		$data_source = call_user_func(array($self, 'data_source'));
		$table = call_user_func(array($self, 'table'));
