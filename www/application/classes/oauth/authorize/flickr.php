...
/**
 * Class OAuth_Authorize_Flickr - OAuth authorize
 */
class OAuth_Authorize_Flickr extends OAuth_Authorize {
    ...
    public function login($providerId, $info) {
        ...
        if($object = json_decode($info, true)) {
            $oauthUserId = null;
            if(isset($object['user_nsid'])) {
                $oauthUserId = $object['user_nsid'];
            }

            if($oauthUserId == null) return;

            $oauthUsername = null;
            if(isset($object['fullname']) && !empty($object['fullname'])) {
                $oauthUsername = $object['fullname'];
            } else if(isset($object['username']) && !empty($object['username'])) {
                $oauthUsername = $object['username'];
            } else {
