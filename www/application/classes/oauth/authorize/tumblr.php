...
/**
 * Class OAuth_Authorize_Tumblr - OAuth authorize
 */
class OAuth_Authorize_Tumblr extends OAuth_Authorize {
    ...
    public function login($providerId, $info) {
        ...
        if($object = json_decode($info, true)) {
            $oauthUserId = null;
            if(isset($object['response']) && isset($object['response']['user']) && isset($object['response']['user']['name'])) {
                $oauthUserId = $object['response']['user']['name'];
            }

            if($oauthUserId == null) return;

            $oauthUsername = $oauthUserId;
