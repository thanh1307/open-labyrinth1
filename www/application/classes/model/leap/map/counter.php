...
class Model_Leap_Map_Counter extends Model_Leap_Base
{
    ...
    public function getMainCounterFromSessionTrace($trace)
    {
        ...
        $result = preg_match('#(\[CID=' . $main_counter['id'] . ')+(,V=)+(?<value>' . $float_or_string . '?)?(\])+#',
            $counters, $matches);
    ...
    public function progress($points, $map_id)
    {
        ...
        $main_counter_id = DB_SQL::select('default')->from($this->table())->where('map_id', '=', $map_id,
            'AND')->where('status', '=', 1)->query();

        // get max value and id of main counter
        preg_match('/(MCID=)(?<id>\d+),V=(?<max_value>\d+)/', $points, $main_counter);
        if ($main_counter_id AND (Arr::get(Arr::get($main_counter_id, 0, array()), 'id') == Arr::get($main_counter,
                    'id', 'not_exist'))
        ) {
    ...
    private function try_become_a_main($status, $map_id)
    {
        ...
        foreach (DB_SQL::select('default')->from($this->table())->where('map_id', '=', $map_id, 'AND')->where('status',
            '=', $status)->query() as $obj) {
