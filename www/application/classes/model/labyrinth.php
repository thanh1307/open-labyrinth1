...
class Model_Labyrinth extends Model
{
    ...
    public function execute($nodeId, $bookmark = null, $isRoot = false, $cumulative = false)
    {
        ...
        if ($node) {
            ...
            if ($scenarioSectionId) {
                foreach (DB_ORM::select('Map_Node_Section_Node')->where('section_id', '=',
                    $scenarioSectionId)->query()->as_array() as $sectionObj) {
    ...
    private function render_c_debug($data)
    {
        ...
        foreach ($data as $id_c => $counter) {
            ...
            foreach (Session::instance()->get('c_debug', array()) as $id_p => $popup) {
                foreach ($popup as $id_counter => $v_popup) {
                    if ($id_counter == $id_c) {
                        ...
                        $v_previous .= '<span class="colored-bl popup-color"></span>Popup #' . $id_p . ': ' . $cmr . $this->check_sign($v_popup,
                                'popup-color') . '<br>';
                        $cmr += $v_popup;
                    }
                }
            ...
            // -- question value -- //
            if ($v_question) {
                $result[$id_c]['info'] .= '<span class="colored-bl question-color"></span>Question #' . $counter['question_id'] . ': ' . $cmr . $this->check_sign($v_question,
                        'question-color') . '<br>';
                $cmr += $v_question;
            }
            // -- counter value -- //
            if ($v_counter) {
                $result[$id_c]['info'] .= '<span class="colored-bl counter-color"></span>Counter: ' . $cmr . $this->check_sign($v_counter,
                        'counter-color') . '<br>';
                $cmr += $v_counter;
            }
            // -- rule value -- //
            ...
            foreach ($global_rules as $id_g_r => $g_rule) {
                ...
                foreach (Arr::get($g_r_result, 'counters', array()) as $id_c_g_r => $outcome) {
                    ...
                    if ($v_global_rule) {
                        $result[$id_c]['info'] .= '<span class="colored-bl global-rule-color"></span>Global rule #' . $id_g_r . ': ' . $cmr . $this->check_sign($v_global_rule,
                                'global-rule-color') . '<br>';
                        $cmr += $v_global_rule;
                    }
                }
            }
    ...
    private function checkUser($mapId, $allowReviewers = false)
    {
        ...
        if (Auth::instance()->logged_in() AND $user->type->name != 'learner') {
            ...
            if ((DB_ORM::model('map_user')->checkUserById($mapId, $user->id)) OR
                ...
                ($allowReviewers AND $user->type->name == 'reviewer')
            ) {
                return true;
            }
        }
        ...
        if (count($draggingQuestionResponses)) {
            ...
            foreach ($draggingQuestionResponses as $responseJSON) {
                ...
                if (isset($responseObject['id']) AND isset($responseObject['responses'])) {
                    DB_ORM::model('user_response')->createResponse($sessionId, $responseObject['id'],
                        json_encode($responseObject['responses']), $nodeId, $created_at);
                }
            }
        ...
        if ($traceId AND $node) {
            ...
            if (count($counters)) {
                ...
                if ($cumulative) {
                    ...
                    if ($sessionObj) {
                        $sessionTrace = DB_ORM::select('User_SessionTrace')->where('session_id', '=',
                            $sessionObj->id)->query()->fetch(0);

                        $countersTrace = $sessionTrace->counters;
                        DB_ORM::update('User_SessionTrace')->set('counters', $countersTrace)->where('session_id', '=',
                            $sessionId)->execute();
                ...
                if ($this->bookmark) {
                    ...
                    foreach ($counters as $counter) {
                        ...
                        $currentCountersState = DB_ORM::model('user_sessionTrace')->getCounterByIDs($sessionId,
                            $rootNode->map_id, $rootNode->id);
                        ...
                        if ($isRoot) {
                            ...
                            if ($if_main) {
                                $main_counter['value'] = $thisCounter;
                            }
                            ...
                            if ($if_main) {
                                preg_match('/(MCID=)(?<id>\d+),V=(?<value>\+?\-?\d+)/', $currentCountersState,
                                    $matches);
                    ...
                    if (count($commonRules)) {
                        ...
                        foreach ($commonRules as $rule) {
                            if (!in_array('RULE_' . $rule->id, $stopRules)) {
                                ...
                                preg_match_all('/QU_ANSWER:(?<id>[^\]]*)\]\]\s*,\s*\'(?<response>[^\']*)/', $rule->rule,
                                    $matches);
        ...
        if ($conditionChangeObj) {
            ...
            foreach ($conditionChangeObj as $changeObj) {
                ...
                if ($changeObj->appears) {
                    $conditionString .= '<li>' . DB_ORM::model('Conditions',
                            array($changeObj->condition_id))->name . ' (' . $result . ')</li>';
                }
            }
            $conditionString .= '</ul>';
        }
    ...
    public function review($nodeId)
    {
        ...
        if ($sessionId AND $nodeId) {
            ...
            if ($node) {
                ...
                $counter = DB_ORM::model('user_sessionTrace')->getCounterByIDs($sessionId, (int)$node->map_id,
                    $node->id);
                if (isset($rootNode->id)) {
            ...
            if (count($chat->elements) > 0) {
                foreach ($chat->elements as $element) {
                    if ($element->id == $elementId) {
                        ...
                        $counterStr = DB_ORM::model('user_sessionTrace')->getCounterByIDs($sessionId,
                            (int)$rootNode->map_id, $rootNode->id);
                                        ...
                                        if ($chat->counter_id == $counter->id) {
                                            if ($element->function != '') {
                                                ...
                                                if ($element->function[0] == '=') {
                                                    $tmpCounter = (int)substr($element->function, 1,
                                                        strlen($element->function));
                                                } else {
                                                    if ($element->function[0] == '-') {
                                                        $tmpCounter -= (int)substr($element->function, 1,
                                                            strlen($element->function));
                                                ...
                                                $counterStr = str_replace('[CID=' . $counter->id . ',V=' . $thisCounter . ']',
                                                    '[CID=' . $counter->id . ',V=' . $tmpCounter . ']', $counterStr);
                                                DB_ORM::model('user_sessionTrace')->updateCounter($sessionId,
                                                    $rootNode->map_id, $rootNode->id, $counterStr);
    ...
    public function question($questionId, $response, $questionStatus, $nodeId)
    {
        ...
        if ($question) {
            ...
            if (($question->type->value == 'text') OR ($question->type->value == 'area')) {
                ...
                DB_ORM::model('user_response')->createResponse($this->getSessionID(), $question->id, $response,
                    $nodeId);
                        ...
                        if (count($counters) > 0) {
                            foreach ($counters as $counter) {
                                if ($counter[2] == $rule->counter_id) {
                                    if (Model_Labyrinth::calculateRule($rule->operator->value, $counter[1][0],
                                        $rule->value)
                                    ) {
    ...
    public function getCounterString($mapId)
    {
        ...
        if ($mapId != null) {
            $rootNode = DB_ORM::model('map_node')->getRootNodeByMap($mapId);
            if ($rootNode != null) {
                $currentCountersState = DB_ORM::model('user_sessionTrace')->getCounterByIDs($this->getSessionID(),
                    $rootNode->map_id, $rootNode->id);
            }
        }
    ...
    public function updateCounterString($mapId, $string)
    {
        if ($mapId != null) {
            $rootNode = DB_ORM::model('map_node')->getRootNodeByMap($mapId);
            if ($rootNode != null) {
                DB_ORM::model('user_sessionTrace')->updateCounter($this->getSessionID(), $rootNode->map_id,
                    $rootNode->id, $string);
            }
        }
