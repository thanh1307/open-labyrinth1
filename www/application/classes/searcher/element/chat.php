...
class Searcher_Element_Chat extends Searcher_Element {
    private $mapId;
    private $fields;
    ...
    public function search($searchText) {
        ...
        $builder = DB_SQL::select('default')
                           ->column('M.id', 'id', true)
                           ->from(Model_Leap_Map_Chat::table(), 'M');
        $checkForResponse = $this->checkForQuestions();
        if($checkForResponse) {
            $builder->join('LEFT OUTER', Model_Leap_Map_Chat_Element::table(), 'R');
            $builder->on('M.id', '=', 'R.chat_id', true);
        }
        ...
        if($records->is_loaded()) {
            foreach($records as $record) {
                $modelObject = DB_ORM::model('map_chat', array((int)$record['id']));
                $isAdd = false;
                ...
                if($isAdd) {
                    ...
                    $data[] = new Searcher_Result('chat', URL::base() . 'chatManager/editChat/' . $this->mapId . '/' . $modelObject->id, $modelObject->stem, $content, $searchText);
                }
            }
        }
    ...
    private function checkForQuestions() {
