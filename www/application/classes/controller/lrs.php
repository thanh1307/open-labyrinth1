...
class Controller_LRS extends Controller_Base
{
    ...
    {
        $this->showView();
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Thêm LRS')));
    }
    ...
    public function action_update()
    {
        ...
        $this->showView($id);
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Chỉnh sửa LRS')));
    }
    ...
    public function action_sendFailedLRSStatements()
    {
        ...
        Session::instance()->set('info_message', 'Nội dung đã được gửi tới LRS');
        Request::initial()->redirect(URL::base() . 'lrs/failedStatements');
    }
    ...
    public function action_deleteFailedLRSStatements()
    {
        ...
        Session::instance()->set('info_message', 'Đã xóa nội dung.');
        Request::initial()->redirect(URL::base() . 'lrs/failedStatements');
    }
    ...
    public function action_sendSelectedFailedLRSStatements()
    {
        ...
        if (empty($lrs_statement_ids)) {
            Session::instance()->set('error_message', 'Nội dung chưa được chọn.');
            die;
        }
        /** @var Model_Leap_LRSStatement[] $lrs_statements */
        $lrs_statements = DB_ORM::select('LRSStatement')
    ...
    public function action_deleteSelectedFailedLRSStatements()
    {
        ...
        Session::instance()->set('info_message', 'Nội dung đã bị xóa.');
        Request::initial()->redirect(URL::base() . 'lrs/failedStatements');
    }
    ...
    public function action_sendReportSubmit()
    {
        ...
        if (empty($webinar_id)) {
            Session::instance()->set('error_message', 'Webinar_ID không thể trống');
            $this->jsonResponse(array('completed' => true));
        }

        if (empty($date_from) || empty($date_to)) {
            Session::instance()->set('error_message', 'Ngày tháng không thể trống');
            $this->jsonResponse(array('completed' => true));
        }
        ...
        if ($is_initial_request && $sessions->count() == 0) {
            Session::instance()
                ->set('error_message', 'Không tìm thấy phiên cho phạm vi ngày: ' . $date_from . ' - ' . $date_to);
