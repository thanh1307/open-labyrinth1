...
class Controller_RenderLabyrinth extends Controller_Template
{
    ...
    public function action_saveVideoXAPIStatements()
    {
        ...
        if (empty($session_id)) {
            throw new Exception('session_id không thể để trống');
        }
    ...
    private function renderLabyrinth($action)
    {
        ...
        if ($action == 'index') {
            ...
            if (empty($sessionId) || !empty($endTime)) {
                Session::instance()->set('finalSubmit', 'Bản đồ đã được hoàn thành, bạn không thể thay đổi câu trả lời của mình');
                Request::initial()->redirect(URL::base());
            }
            ...
            if ($node != null) {
                ...
                if ($data) {
                    ...
                    if ($data) {
                        ...
                        $data['trace_links'] = '<a href="#" onclick="toggle_visibility(' . "'track'" . ');"><p class="style2"><strong>Xem lại lộ trình của bạn</strong></p></a><div id="track" style="display:none">' . $data['trace_links'] . '</div>';
                        $rootNode = DB_ORM::model('map_node')->getRootNodeByMap((int)$mapId);
                        $out = "<labyrinth>";
    ...
    private function generateNavigation($sections)
    {
        ...
        if (count($sections) > 0) {
            ...
            foreach ($sections as $section) {
                ...
                } elseif ($section->map->section->name == 'Có thể điều hướng') {
                    $result .= '<li><a href="';
        ...
        if ($question) {
            ...
            if ($q_type == 'text') {
                ...
                if ($question->show_submit == 1) {
                    ...
                    $result .= '/>
                        <span id="questionSubmit' . $question->id . '" style="display:none;font-size:12px">Câu trả lời đã được gửi</span>
