...
class Controller_AvatarManager extends Controller_Base {
    ...
    public function action_index() {
        ...
        if ($mapId != NULL) {
            ...
            Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Ảnh đại diện'))->set_url(URL::base() . 'avatarManager/index/' . $mapId));
    ...
    public function action_deleteAvatar() {
        ...
        if ($mapId != NULL and $avatarId != NULL) {
            ...
            if($references != NULL){
                ...
                $ses->set('warningMessage', 'Hình đại diện chưa bị xóa. Hình đại diện đã chọn sẽ được sử dụng trong các Hệ thống sau:');
            } else {
    ...
    public function action_updateAvatar() {
        ...
        if ($_POST and $mapId != NULL and $avatarId != NULL) {
            ...
            if($references != NULL && $privete){
                $ses = Session::instance();
                $ses->set('listOfUsedReferences', CrossReferences::getListReferenceForView($references));
                $ses->set('warningMessage', 'Hình đại diện chưa được đặt ở chế độ riêng tư. Hình đại diện đã chọn sẽ được sử dụng trong các Hệ thống sau:');
    ...
    public function action_globalAvatars(){
        ...
        if ($mapId != NULL) {
            ...
            Breadcrumbs::add(Breadcrumb::factory()->set_title($this->templateData['map']->name)->set_url(URL::base() . 'labyrinthManager/global/' . $mapId));
            Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Ảnh đại diện toàn cục'))->set_url(URL::base() . 'avatarManager/index/' . $mapId));
    ...
    public function action_exportAvatar(){
        ...
        if ($mapId != NULL && $avatarId!= NULL) {
            ...
            if(mkdir($tmpfolder)){
                ...
                header("Kiểm soát bộ đệm: công khai");
                header("Nội dung-Mô tả: Truyền tệp");
                header("Bố trí nội dung: tệp đính kèm; filename=" . $pathInfo['basename']);
                header("Loại nội dung: application/zip");
                header("Mã hóa nội dung: binary");
                readfile('tmp/' . $rand . '.zip');
                unlink($zipFile);
            } else Request::initial()->redirect(URL::base() . 'avatarManager/index/' . $mapId);
    ...
    public function action_importAvatar(){
        ...
        if($mapId != NULL && is_file('global/avatars/' . $xml)){
            ...
            $fields = array('avskin1'       => $data->skin_1,
                          'avskin2'         => $data->skin_2,
                          'avcloth'         => $data->cloth,
                          'avnose'          => $data->nose,
                          'avhair'          => $data->hair,
