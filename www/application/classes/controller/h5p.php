...
class Controller_H5P extends Controller_Base
{
    ...
    public function before()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Quản lý H5P'))->set_url(URL::base() . 'h5p/index'));
    }
    ...
    public function action_ajax_files()
    {
        ...
        if (!$file->isLoaded()) {
            H5PCore::ajaxError(__('Không tìm thấy tập tin trên máy chủ. Kiểm tra cài đặt tải lên tập tin.'));
            die;
        }
    ...
    public function action_deleteTemporaryFiles()
    {
        ...
        if (!file_exists($temp_dir) || ((time() - filemtime($temp_dir)) < 60 * 60 * 24)) {
            Session::instance()->set('success_message', __('Không có tệp tin để xóa.'));
            Request::initial()->redirect(URL::base() . 'h5p');
        }

        FileHelper::removeDirectory($temp_dir);

        Session::instance()->set('success_message', __('Đã xóa.'));
        Request::initial()->redirect(URL::base() . 'h5p');
    }
    ...
    public function action_libraryUpload()
    {
        if (!isset($_FILES['h5p_file'])) {
            Session::instance()->set('error_message', __('Tệp tin không được chọn.'));
        ...
        if ($result === false) {
            Session::instance()->set('error_message', __('Lỗi.'));
        } else {
            Session::instance()->set('success_message', __('Đã tải lên.'));
        }
    ...
    {
        if ($this->request->method() !== 'POST') {
            Session::instance()->set('error_message', __('Yêu cầu phương pháp sai'));
            Request::initial()->redirect(URL::base() . 'h5p/libraries');
        }
        ...
        // chua fix 12-11-23
        // if (!$library) {
        //     Session::instance()->set('error_message', __('Không tìm thấy thư viện'.'));
        //     Request::initial()->redirect(URL::base() . 'h5p/libraries');
        // }
        ...
        if ($usage['content'] !== 0 || $usage['libraries'] !== 0) {
            Session::instance()->set('error_message',
                __('Thư viện này được sử dụng bởi nội dung hoặc các thư viện khác và do đó không thể xóa được.'));
            Request::initial()->redirect(URL::base() . 'h5p/libraries');
        }
    ...
    public function action_libraryDelete()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Các thư viện'))->set_url(URL::base(true) . 'h5p/libraries'));
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Thư viện') . ' ' . $library->title)->set_url(URL::base(true) . 'h5p/libraryShow/' . $id));
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Xóa')));
    ...
    public function action_libraryShow()
    {
        ...
        $settings['libraryInfo']['translations'] = array(
            'noContent' => __('Không có nội dung nào được sử dụng trong thư viện này'),
            'contentHeader' => __('Nội dung đang được sử dụng trong thư viện'),
            'pageSizeSelectorLabel' => __('Các phần tử trên mỗi trang'),
            'filterPlaceholder' => __('Lọc nội dung'),
            'pageXOfY' => __('Trang $x của $y'),
        );
        ...
        $settings['libraryInfo']['info'] = array(
            __('Phiên bản') => H5PCore::libraryVersion($library),
            __('Toàn màn hình') => $library->fullscreen ? __('Có') : __('Không'),
            __('Nội dung thư viện') => $library->runnable ? __('Có') : __('Không'),
            __('Được sử dụng bởi') => (isset($contents) ? sprintf(__('%d contents'), count($contents)) : __('N/A')),
        );
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Các thư viện'))->set_url(URL::base(true) . 'h5p/libraries'));
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Thư viện') . ' ' . $library->title));
    ...
    public function action_saveXAPIStatement()
    {
        ...
        if (empty($session_id)) {
            throw new Exception('ID_Session không được để trống');
        }
    ...
    public function action_saveResult()
    {
        ...
        if (!$content_id) {
            H5PCore::ajaxError(__('Nội dung không hợp lệ.'));
            die;
        }
        ...
        if (empty($user)) {
            H5PCore::ajaxError(__('Người dùng không được ủy quyền.'));
            die;
        }
    ...
    public function action_embed()
    {
        ...
        echo '<body style="margin:0"><div style="background: #fafafa url(/images/h5p/h5p.svg) no-repeat center;background-size: 50% 50%;width: 100%;height: 100%;"></div><div style="width:100%;position:absolute;top:75%;text-align:center;color:#434343;font-family: Consolas,monaco,monospace">' . __('Nội dung không có sẵn.') . '</div></body>';
        die;
    }
    ...
    public function action_showContent()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Nội dung') . ' #' . $id));
    ...
    public function action_results()
    {
        ...
        if (is_string($content_admin->content)) {
            ...
            $settings = $plugin_admin->get_data_view_settings(
                ...
                array(true),
                __("Không có kết quả được ghi lại cho nội dung này."),
                (object)array(
                    'by' => 4,
                    'dir' => 0
                )
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Nội dung') . ' #' . $id)->set_url(URL::base() . 'h5p/showContent/' . $id));
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Kết quả')));
    ...
    public function action_addContent()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Nội dung') . (!empty($id) ? ' #' . $id : '')));
    ...
    public function action_addContentSubmit()
    {
        ...
        if ($action) {
            ...
            if ($action === 'create') {
                ...
                if (!isset($_FILES['h5p_file'])) {
                    Session::instance()->set('error_message', __('Tập tin không được chọn.'));
                    Request::initial()->redirect(URL::base() . 'h5p/addContent');
                }
            ...
            if ($result) {
                ...
                Session::instance()->set('success_message', 'Đã lưu.');
            } else {
    ...
    public function action_contentDelete()
    {
        ...
        if (empty($id)) {
            Session::instance()->set('error_message', 'ID không thể để trống');
            Request::initial()->redirect(URL::base() . 'h5p/addContent');
        }
        ...
        Session::instance()->set('success_message', 'Đã xóa.');
        Request::initial()->redirect(URL::base() . 'h5p/');
    }
    ...
    public function action_index()
    {
        ...
        $data_view = $plugin_admin->get_data_view_settings(
            ...
            $headers,
            array(true),
            __("Không có nội dung H5P. Bạn phải tải lên hoặc tạo nội dung mới."),
            (object)array(
                'by' => 4,
                'dir' => 0
            )
    ...
    public function action_libraries()
    {
        ...
        $settings = array(
            ...
            'l10n' => array(
                'NA' => __('N/A'),
                'viewLibrary' => __('Xem chi tiết thư viện'),
                'deleteLibrary' => __('Xóa thư viện'),
                'upgradeLibrary' => __('Cập nhật nội dung thư viện')
            )
        );
        ...
        $settings['libraryList']['listHeaders'] = array(
            ...
            array(
                'text' => __('Nội dung sử dụng'),
                'class' => 'h5p-admin-center'
            ),
            array(
                'text' => __('Thư viện sử dụng'),
                'class' => 'h5p-admin-center'
            ),
            __('Hành động')
        );
        ...
        if ($needsUpgrade !== '') {
          ...
          <p>' . __('Các thư viện sau đã lỗi thời và cần được nâng cấp:') . '</p>
          <ul id="h5p-outdated">' . $needsUpgrade . '</ul>
          <p>' . __('Để nâng cấp tất cả các thư viện đã cài đặt, hãy làm như sau:') . '</p>
          <ol>
            <li>' . sprintf(__('Tải file H5P từ %s trang.'),
                    '<a href="https://h5p.org/update-all-content-types">Nâng Cấp Tất Cả Loại Nội Dung</a>') . '</li>
            <li>' . sprintf(__('Chọn loại tập tin đã tải <em> %s</em> trong tập tin dưới đây.'), 'upgrades.h5p') . '</li>
            <li>' . __('Chọn "Chỉ cập nhật thư viện hiện có" và nhấp vào nút <em>Upload</em>.') . '</li>
          </ol> </p>'
            );
        }
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Thư viện')));
    ...
    private function get_not_cached_settings($num)
    {
        return array(
            ...
            'message' => __('Không phải tất cả nội dung đều được xây dựng lại bộ nhớ đệm. Điều này là bắt buộc để có thể xóa thư viện và hiển thị số lượng nội dung sử dụng thư viện.'),
            'progress' => sprintf(__('%d nội dung cần được xây dựng lại bộ đệm.'), $num),
            'button' => __('Xây lại')
        );
    }
    ...
    private function get_contents_insert_row($result)
    {
        return array(
            ...
            '<button class="button h5p-insert" data-id="' . $result->id . '" data-slug="' . $result->slug . '">' . __('Chèn') . '</button>'
        );
    }
    ...
    private function get_contents_row($result)
    {
        ...
        if (get_option('h5p_track_user', true)) {
            $row[] = '<a href="/h5p/results/' . $result->id . '">' . __('Kết quả') . '</a>';
        }

        // Add edit link
        $row[] = '<a href="/h5p/addContent/' . $result->id . '">' . __('Chỉnh sửa') . '</a>';
