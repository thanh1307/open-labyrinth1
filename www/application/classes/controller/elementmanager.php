...
class Controller_ElementManager extends Controller_Base {
    ...
    public function action_index() {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Phần tử'))->set_url(URL::base() . 'elementManager/index/' . $mapId));
    ...
    public function action_addNewElement()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title($this->templateData['map']->name)->set_url(URL::base() . 'labyrinthManager/global/' . $mapId));
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Phần tử'))->set_url(URL::base() . 'elementManager/index/' . $mapId));
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Mới'))->set_url(URL::base() . 'elementManager/addNewElement/' . $mapId));
    ...
    public function action_updateElement() {
        ...
        if ($_POST and $mapId != NULL and $vpdId != NULL) {
            ...
            if($private && $references != NULL){
                ...
                $ses->set('warningMessage', 'Phần tử này không được đặt ở chế độ riêng tư. Phần tử được chọn sẽ được sử dụng trong Hệ thống sau:');
                $_POST['Private'] = 'Off';
            }
    ...
    public function action_deleteVpd()
    {
        ...
        if ($mapId != NULL and $vpdId != NULL)
        {
            ...
            if($references != NULL){
                $ses = Session::instance();
                $ses->set('listOfUsedReferences', CrossReferences::getListReferenceForView($references));
                $ses->set('warningMessage', 'Phần tử này chưa được xóa. Phần tử được chọn sẽ được sử dụng trong Hệ thống sau:');
            } else {
