...
class Controller_ExportImportManager extends Controller_Base {
    ...
    public function action_exportVUE() {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Xuất VUE'))->set_url(URL::base() . 'exportimportmanager/exportVUE/id/' . $mapId));
    ...
    public function action_importVUE() {
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Thêm VUE'))->set_url(URL::base() . 'exportimportmanager/importVUE'));
    ...
    public function action_import() {
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Thêm MedBiquitous VP'))->set_url(URL::base().'exportimportmanager/import'));
    ...
    {
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Xuất Medbiquitous ANSI'))->set_url(URL::base() . 'exportimportmanager/exportMVP'));
    ...
    public function exportVUE($mapId) {
        if ($mapId != NULL) {
            ...
            $f = fopen($fileName, 'w') or die("Không thể tạo file");
            fwrite($f, $out);
            fclose($f);
        }
    }
    ...
    public function action_upload()
    {
        ...
        try {
            ...
            Notice::add('Labyrinth <a target="_blank" href="'.URL::base().'labyrinthManager/info/'.$data["id"].'">'.$data["title"].'</a> được thêm mới thành công.', 'success');
        } catch (Exception $e) {
            $message = $e->getMessage();
            Notice::add("Lỗi, hệ thống không được thêm đúng cách.".PHP_EOL.$message, 'error');
