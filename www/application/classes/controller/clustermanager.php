...
class Controller_ClusterManager extends Controller_Base {
    ...
    public function action_index()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Nhóm'))->set_url(URL::base().'clusterManager/index/'.$mapId));
    }
    ...
    public function action_addDam() {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title($this->templateData['map']->name)->set_url(URL::base().'labyrinthManager/global/'.$mapId));
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Nhóm'))->set_url(URL::base().'clusterManager/index/'.$mapId));
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Thêm mới'))->set_url(URL::base().'clusterManager/addDam/'.$mapId));
    }
    ...
    public function action_deleteDam()
    {
        ...
        if ($damId AND $mapId)
        {
            ...
            if($references != NULL)
            {
                ...
                $ses->set('warningMessage', 'Lớp học không bị xóa. Lớp học đã chọn sẽ được sử dụng trong các Hệ thống sau:');
            }
    ...
    public function action_editCluster()
    {
        ...
        if ($mapId AND $damId)
        {
            ...
            {
                $this->templateData['warningMessage'] = 'Lớp không được đặt ở chế độ riêng tư. Vui lòng kiểm tra các Hệ thống khác để tham khảo trên lớp này.';
    ...
    public function action_updateDamName() {
        ...
        if ($_POST and $mapId != NULL and $damId != NULL) {
            ...
            if($references != NULL && $privete){
                $ses = Session::instance();
                $ses->set('listOfUsedReferences', CrossReferences::getListReferenceForView($references));
                $ses->set('warningMessage', 'Lớp không được đặt ở chế độ riêng tư. Lớp đã chọn sẽ được sử dụng trong các Hệ thống sau:');
