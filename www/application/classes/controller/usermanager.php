...
class Controller_UserManager extends Controller_Base
{
    ...
    public function before()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Quản lý người dùng'))->set_url(URL::base() . 'usermanager'));
    ...
    private function sendNewUserMail($userData)
    {
        ...
        $mail_body = 'Chào mừng đến với Hệ thống hỗ trợ học tập, ' . $userData['uname'] . '!' . PHP_EOL . PHP_EOL;
        $mail_body .= 'Sau đây là thông tin về tài khoản của bạn:' . PHP_EOL;
        $mail_body .= '---------------------------------------' . PHP_EOL;
        $mail_body .= 'Tài khoản: ' . $userData['uid'] . PHP_EOL;
        $mail_body .= 'Mật khẩu: ' . $userData['upw'] . PHP_EOL;
        $mail_body .= 'Họ tên đầy đủ: ' . $userData['uname'] . PHP_EOL;
        $mail_body .= 'Ngôn ngữ: ' . $langName->name . PHP_EOL;
        $mail_body .= 'Loại người dùng: ' . $typeName->name . PHP_EOL;
        $mail_body .= '---------------------------------------' . PHP_EOL;
        $mail_body .= 'URL trang: ' . $URL;
    ...
    public function action_saveNewUser()
    {
        if (!empty($_POST)) {
            ...
            if ((!empty($_POST['uid'])) & (!$checkUserName) & (!$checkUserEmail)) {
                ...
                $error = array();
                if (empty($_POST['uid'])) {
                    $error[] = __('Không được để trống tên người dùng.');
                } elseif ($checkUserName) {
                    $error[] = __('Tên người dùng đã tồn tại.');
                }
                if ($checkUserEmail) {
                    $error[] = __('Địa chỉ email đã tồn tại.');
                }
    ...
    public function manageUser($action)
    {
        ...
        switch ($action) {
            case 'edit':
                Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Chỉnh sửa người dùng') . ' ' . $user->nickname)->set_url(URL::base() . 'usermanager/editUser/' . $userId));
                ...
                break;
            case 'view':
                Breadcrumbs::add(Breadcrumb::factory()->set_title(__('User') . ' ' . $user->nickname)->set_url(URL::base() . 'usermanager/ViewUser/' . $userId));
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            if ($userEmail != $newEmail) {
                ...
                if (!$checkUserEmail) {
                    ...
                    Session::instance()->set('errorMsg', __('Email đã tồn tại.'));
    ...
    {
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Thêm nhóm'))->set_url(URL::base() . 'usermanager/addGroup'));
    ...
    public function action_editGroup()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Chỉnh sửa nhóm'))->set_url(URL::base() . 'usermanager/editGroup/' . $groupId));
