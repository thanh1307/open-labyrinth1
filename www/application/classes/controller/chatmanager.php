...
class Controller_ChatManager extends Controller_Base {
    ...
    public function action_index()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Nhắn tin'))->set_url(URL::base() . 'chatManager/index/' . $mapId));
    ...
    public function action_addChat()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title($this->templateData['map']->name)->set_url(URL::base() . 'labyrinthManager/global/' . $mapId));
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Nhắn tin'))->set_url(URL::base() . 'chatManager/index/' . $mapId));
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Thêm mới'))->set_url(URL::base() . 'chatManager/addChat/' . $mapId));
    ...
    public function action_deleteChat()
    {
        ...
        if ($mapId AND $chatId)
        {
            ...
            if ($references != NULL)
            {
                ...
                $ses->set('warningMessage', 'Cuộc trò chuyện không bị xóa. Trò chuyện đã chọn sẽ được sử dụng trong Hệ thống sau:');
            } else {
    ...
    public function action_updateChat() {
        ...
        if ($_POST and $mapId != NULL and $chatId != NULL) {
            ...
            if($references != NULL && $privete){
                $ses = Session::instance();
                $ses->set('listOfUsedReferences', CrossReferences::getListReferenceForView($references));
                $ses->set('warningMessage', 'Cuộc trò chuyện không được đặt ở chế độ riêng tư. Trò chuyện đã chọn sẽ được sử dụng trong Hệ thống sau:');
