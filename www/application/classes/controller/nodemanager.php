...
class Controller_NodeManager extends Controller_Base {
    ...
    public function action_index()
    {
        ...
        if ($mapId)
        {
            ...
            Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Nút'))->set_url(URL::base() . 'nodeManager/index/' . $mapId));
    ...
    public function action_updateNode() {
        ...
        if($references != NULL && $privete){
            ...
            $ses->set('warningMessage', 'Nút không được đặt ở chế độ riêng tư. Phần có thông tin hỗ trợ của nút đã chọn được sử dụng trong hệ thống sau:');
    ...
    public function action_deleteNode()
    {
        ...
        if($references != NULL){
            $ses = Session::instance();
            $ses->set('listOfUsedReferences', CrossReferences::getListReferenceForView($references));
            $ses->set('warningMessage', 'Nút chưa bị xóa. Phần có thông tin hỗ trợ của nút đã chọn được sử dụng trong hệ thống sau:');
        } else {
    ...
    public function action_editSection()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title($this->templateData['map']->name)->set_url(URL::base() . 'labyrinthManager/global/' . $mapId));
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Phần'))->set_url(URL::base() . 'nodeManager/sections/' . $mapId));
