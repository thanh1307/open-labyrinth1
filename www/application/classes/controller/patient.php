...
class Controller_Patient extends Controller_Base {
    ...
    public function action_index()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Hệ thống của tôi'))->set_url(URL::base().'patient/index/'));
    }
    ...
    public function action_labyrinth()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Người dùng'))->set_url(URL::base().'patient/labyrinth/'.$id_map));
    }
    ...
    public function action_connectionManage ()
    {
        ...
        $sameA      = DB_ORM::select('Patient')->where('type', '=', 'Bộ song song tương tự')->query()->as_array();
        $sameB      = DB_ORM::select('Patient')->where('type', '=', 'Bộ theo cùng chiều dọc')->query()->as_array();
        $differentA = DB_ORM::select('Patient')->where('type', '=', 'Bộ song song khác nhau')->query()->as_array();
        $differentB = DB_ORM::select('Patient')->where('type', '=', 'Bộ chiều dọc khác nhau')->query()->as_array();
