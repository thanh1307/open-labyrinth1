...
class Controller_QuestionManager extends Controller_Base {
    ...
    public function action_index() {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Câu hỏi'))->set_url(URL::base() . 'questionManager/index/' . $mapId));
    }
    ...
    public function action_question()
    {
        ...
        if ($questionId) {
            ...
            Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Chỉnh sửa'))->set_url(URL::base().'questionManager/question/'.$mapId.'/'.$typeId.'/'.$questionId));
        } else {
            Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Mới'))->set_url(URL::base().'questionManager/question/'.$mapId.'/'.$typeId));
        }
    ...
    public function action_questionPOST()
    {
        ...
        if ($questionId) {
            ...
            if ($references AND $private) {
                ...
                $ses->set('warningMessage', 'Câu hỏi chưa được đặt ở chế độ riêng tư. Câu hỏi đã chọn sẽ được sử dụng trong hệ thống sau:');
    ...
    public function action_deleteQuestion()
    {
        ...
        if ($mapId != NULL and $questionId != NULL)
        {
            ...
            if($references != NULL){
                $ses = Session::instance();
                $ses->set('listOfUsedReferences', CrossReferences::getListReferenceForView($references));
                $ses->set('warningMessage', 'Câu hỏi chưa được xóa. Câu hỏi đã chọn sẽ được sử dụng trong hệ thống sau:');
            } else {
    ...
    public function action_globalQuestions()
    {
        ...
        if ($mapId) {
            ...
            Breadcrumbs::add(Breadcrumb::factory()->set_title($this->templateData['map']->name)->set_url(URL::base() . 'labyrinthManager/global/' . $mapId));
            Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Câu hỏi toàn cục'))->set_url(URL::base() . 'questionManager/index/' . $mapId));
        } else {
            Request::initial()->redirect(URL::base());
        }
    }
