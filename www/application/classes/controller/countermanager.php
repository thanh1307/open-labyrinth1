...
class Controller_CounterManager extends Controller_Base
{
    ...
    public function action_index()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Bộ đếm'))->set_url(URL::base() . 'counterManager/index/' . $mapId));
    }
    ...
    public function action_addCounter()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title($this->templateData['map']->name)->set_url(URL::base() . 'labyrinthManager/global/' . $mapId));
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Bộ đếm'))->set_url(URL::base() . 'counterManager/index/' . $mapId));
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Thêm bộ đếm'))->set_url(URL::base() . 'counterManager/addCounter/' . $mapId));
    }
    ...
    public function action_grid()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title($this->templateData['map']->name)->set_url(URL::base() . 'labyrinthManager/global/' . $mapId));
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Hệ thống đếm'))->set_url(URL::base() . 'counterManager/grid/' . $mapId));
    }
    ...
    public function action_rules()
    {
        ...
        Breadcrumbs::add(Breadcrumb::factory()->set_title($this->templateData['map']->name)->set_url(URL::base() . 'labyrinthManager/global/' . $mapId));
        Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Luật'))->set_url(URL::base() . 'counterManager/rules/' . $mapId));
    }
    ...
    function commonRule()
    {
        ...
        if ($mapId) {
            ...
            Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Luật'))->set_url(URL::base() . 'counterManager/rules/' . $mapId));
            $ruleId
                ? Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Chỉnh sửa luật'))->set_url(URL::base() . 'counterManager/editCommonRule/' . $mapId))
                : Breadcrumbs::add(Breadcrumb::factory()->set_title(__('Thêm mới luật'))->set_url(URL::base() . 'counterManager/addCommonRule/' . $mapId));
        } else {
            Request::initial()->redirect(URL::base());
        }
    }
