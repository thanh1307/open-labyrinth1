    ...
    <div class="form-actions">
        <div class="pull-right">
            <input class="btn btn-large btn-primary" type="submit" name="Submit"
                   value="<?php echo __('Nhập tin nhắn'); ?>" onclick="return CheckForm();">
        </div>
    </div>
        ...
        <tr id="m-<?php echo $message['id'] ; ?>" class="message">
            <td width='165px' style="vertical-align: top;">
               ...
               <?php if (Auth::instance()->get_user()->type->name == 'superuser' || Auth::instance()->get_user()->id == $message['author_id']) { ?>
                    <a href="<?php echo URL::base() . 'dtopicManager/editMessage/' . $templateData['topic']['id'] . '/' .$message['id'] ; ?>" rel="tooltip" title="Chỉnh sửa"><i class="icon-edit"></i> <?php echo __('Chỉnh sửa'); ?></a>
                    <a data-toggle="modal" href="javascript:void(0)" data-target="#delete-message-<?php echo $message['id']; ?>" rel="tooltip" title="Xóa"><i class="icon-trash"></i> <?php echo __('Xóa'); ?></a>
                    <div class="modal hide alert alert-block alert-error fade in" id="delete-message-<?php echo $message['id']; ?>">
                        ...
                        <div class="modal-body">
                            <p><?php echo __('Bạn vừa nhấp vào nút xóa, bạn có chắc chắn muốn tiếp tục xóa tin nhắn này khỏi Chủ đề không?'); ?></p>
                            <p>
                                <a class="btn btn-danger" href="<?php echo URL::base() . 'dtopicManager/deleteMessage/' . $templateData['topic']['id'] . '/' .$message['id'] ; ?>"><?php echo __('Xóa'); ?></a> <button class="btn" data-dismiss="modal" aria-hidden="true">Đóng</button>
                            </p>
                        </div>
                    </div>
               <?php }?>
