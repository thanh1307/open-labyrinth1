...
<div class="page-header">
    <h1><?php echo (isset($templateData['name'])) ? __('Chỉnh sửa diễn đàn') . ' - "' .  $templateData['name'] . '"' : __('Thêm mới diễn đàn'); ?></h1>
</div>
...
<form class="form-horizontal" id="form1" name="form1" method="post"
    ...
    <fieldset class="fieldset">
        <div class="control-group">
            <label for="forumname" class="control-label"><?php echo __('Tên diễn đàn'); ?></label>
        ...
        <div class="control-group">
            <label for="firstmessage" class="control-label"><?php echo __('Tin nhắn đầu tiên'); ?></label>
    ...
    <fieldset class="fieldset">
        <div class="control-group">
            ...
            <div class="controls">
                <label class="radio">
                    <input name="security" type="radio" value="0" <?php if (isset($templateData['security'])) { echo ($templateData['security'] == 0) ? 'checked="checked"' : ''; } else { echo 'checked="checked"'; } ?>><?php echo __('Mở'); ?>
                </label>
            </div>
            <div class="controls">
                <label class="radio">
                    <input name="security" type="radio" value="1" <?php if (isset($templateData['security'])) { echo ($templateData['security'] == 1) ? 'checked="checked"' : ''; } ?>><?php echo __('Bảo mật'); ?>
                </label>
        ...
        <?php if (!isset($templateData['name'])) { ?>
            <div class="control-group">
                <label class="control-label"><?php echo __('Gửi thông báo'); ?></label>
                <div class=" controls">
        ...
        <?php if(isset($templateData['forum_id'])) { ?>
            <a class="btn btn-info" rel="tooltip" title="Thêm mới chủ đề" href="/dtopicManager/addTopic/<?php if (isset($templateData['name'])) echo $templateData['forum_id'];  ?> ">
                <i class="icon-plus-sign"></i>
                <span class="visible-desktop">Thêm chủ đề</span>
            </a>
        <?php } ?>
            ...
            <table id="topics" class="table table-striped table-bordered ">
                ...
                <tr>
                    ...
                    <th style="text-align: left">Tác giả</th>
                    <th style="text-align: left">Ngày sáng tác</th>
                    ...
                    <?php foreach ($templateData['topics'] as $topic) {?>
                        <tr>
                            ...
                            <td>
                                    ...
                                    <a class="btn btn-small btn-info" href="<?php echo URL::base() ?>dTopicManager/editTopic/<?php echo $templateData['forum_id']; ?>/<?php echo $topic['id']; ?>" rel="tooltip" title=Chỉnh sửa chủ đề này"><i class="icon-edit"></i> <?php echo __('Chỉnh sửa'); ?></a>
                                    <a data-toggle="modal" href="javascript:void(0);" data-target="#delete-topic-<?php echo $topic['id'];  ?>" rel="tooltip" title="Xóa chủ đề này" class="btn btn-small btn-danger"><i class="icon-trash"></i> <?php echo __('Xóa'); ?></a>

                                    <div class="modal hide alert alert-block alert-error fade in" id="delete-topic-<?php echo $topic['id'];  ?>">
                                        <div class="modal-header">
                                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                                            <h4 class="alert-heading"><?php echo __('Cảnh báo! Bạn chắc chắn chứ?'); ?></h4>
                                        </div>
                                        <div class="modal-body">
                                            <p><?php echo __('Bạn vừa nhấn vào nút xóa, bạn có chắc chắn muốn tiếp tục xóa chủ đề này không?'); ?></p>
                                            <p>
                                                <a class="btn btn-danger" href="<?php echo URL::base() . 'dtopicManager/deleteTopic/' . $templateData['forum_id'] . '/' . $topic['id'] . '/1' ?>"><?php echo __('Xóa chủ đề'); ?></a> <button class="btn" data-dismiss="modal" aria-hidden="true">Đóng</button>
                                            </p>
                                        </div>
                                    </div>
                            </td>
                ...
                <?php } else { ?>
                    <tr class="info">
                        <td colspan="5">Không có chủ đề nào. Bạn có thể thêm bằng cách nhấp vào nút ở trên.</td>
                    </tr>
                <?php }?>
                </tbody>
            </table>

        <h3>Chỉ định người dùng</h3>
        <table id="users" class="table table-striped table-bordered">
            ...
            <thead>
                <tr>
                    ...
                    <th style="text-align: center">Kiểu xác thực</th>
                    <th style="text-align: center">Chuyên gia</th>
                    <th><a href="javascript:void(0);">Người dùng<div class="pull-right"><i class="icon-chevron-down icon-white"></i></div></a></th>
                </tr>
            </thead>
            <tbody><?php
            ...
            <tr>
                ...
                <th>
                    <a href="javascript:void(0);">
                        Nhóm <div class="pull-right"><i class="icon-chevron-down icon-white"></i></div>
                    </a>
                </th>
            </tr>
            </thead>
    ...
    <div class="form-actions">
        <div class="pull-right">
            <input class="btn btn-large btn-primary" type="submit" name="Submit"
                   value="<?php echo (isset($templateData['name'])) ? 'Chỉnh sửa diễn đàn' : 'Thêm mới diễn đàn'; ?>" onclick="return CheckForm();"></div>
        </div>
    </div>
    <?php if (isset($templateData['name'])) { ?>
...
<script>
    ...
    function CheckForm()
    {
        ...
        {
            alert('Nhập tên diễn đàn');
            return false;
        }
        <?php if (!isset($templateData['name'])){ ?>
        if(tinyMCE.get("firstmessage").getContent() =='')
        {
            alert('Nhập tin nhắn đầu tiên');
            return false;
        }
        <?php } ?>
    }
