...
<div class="page-header">
    <h1><?php echo $id_patient ? 'Chỉnh sửa "'.$name.'"' : 'Tạo người dùng ảo'; ?></h1>
</div>
...
<form class="form-horizontal" action="<?php echo URL::base().'patient/update/'.$id_patient ?>" method="post">
    ...
    <div class="control-group">
        <label>
            <span class="f-col">Tên</span>
            <input type="text" class="no-margin" placeholder="Tên" name="name" value="<?php echo $name; ?>" required>
        </label>
        <label>
    ...
    <fieldset class="fieldset">
        ...
        <div class="condition-control-group">
            ...
            <a href="<?php echo URL::base().'patient/deleteCondition/'.$condition->id; ?>" class="btn btn-danger pull-right"><i class="icon-trash"></i>Xóa</a>
            <label>
                <span class="f-col">Giá trị bắt đầu điều kiện</span>
        ...
        <button type="button" class="btn btn-info add-condition-js"><i class="icon-plus"></i>Thêm</button>
    </fieldset>

    <fieldset class="fieldset">
        <legend>Phân công kịch bản</legend><?php
        foreach (Arr::get($templateData, 'patient_scenarios') as $id=>$id_scenario) { ?>
            <div class="condition-control-group">
            <button type="button" class="btn btn-danger pull-right remove-scenario-js" data-id="<?php echo $id; ?>"><i class="icon-trash"></i>Xóa</button>
                <label>
                    <span class="f-col">Chọn kịch bản</span>
        ...
        <button type="button" class="btn btn-info add-labyrinth-js"><i class="icon-plus"></i>Thêm</button>
    </fieldset>

    <button type="submit" class="btn btn-primary btn-large pull-right"><?php echo ($id_patient) ? 'Lưu' : 'Tạo'; ?></button>
</form>
...
<div class="condition-control-group add-labyrinth-bl" style="display: none;">
    <button type="button" class="btn btn-danger pull-right remove-scenario-js"><i class="icon-trash"></i>Xóa</button>
    <label><?php
        if (isset($templateData['scenario'])) { ?>
            ...
            <span class="no-maps">Để thêm kịch bản, trước hết phải tạo nó.</span><?php
        } ?>
    </label>
</div>
