...
<div class="page-header"><?php
    if ($id_type) { ?>
        <h1>Chỉnh sửa bài tập</h1><?php
    } else { ?>
        <h1>Tạo bài tập</h1><?php
    } ?>
</div>
...
<form class="form-horizontal" action="<?php echo URL::base().'patient/assign_save'; ?>" method="post">
    ...
    <div class="control-group">
        <label>
            <span class="f-col">Đối tượng</span>
    ...
    <fieldset class="fieldset assign-bl-js <?php if ($same) echo 'assign-same'; ?>">
        ...
        <div class="condition-control-group assign-same">
            <label>
                ...
                <input class="assign-type" type="radio" name="assign-type" value="users">Người dùng
            </label>
            <label>
                <span class="f-col"></span>
                <input class="assign-type" type="radio" name="assign-type" value="groups">Nhóm
            </label>
        </div><?php
        }
        ...
        <div class="condition-control-group">
            <label>
                <span class="f-col"></span>
                <input class="assign-type" type="radio" name="<?php echo $record->id.'-assign-type'; ?>" value="users" <?php if ($user_or_not) echo 'đã kiểm tra'; ?>>Người dùng
            </label>
            <label>
                <span class="f-col"></span>
                <input class="assign-type" type="radio" name="<?php echo $record->id.'-assign-type'; ?>" value="groups" <?php if ( ! $user_or_not) echo 'đã kiểm tra'; ?>>Nhóm
            </label>
            <a class="btn btn-danger pull-right dra" href="<?php echo URL::base().'patient/delete_assign_record/'.$id_assign.'/'.$id_u_or_group.'/'.$user_or_not; ?>"><i class="icon-trash"></i>Xóa</a><?php
            if ($user_or_not) { ?>
            <label class="assign-user"><?php
                if (isset($templateData['users'])) { ?>
                    <span class="f-col">Chọn người dùng</span>
                    <select name="assign[][user]"><?php
                    foreach ($templateData['users'] as $user) { ?>
                        <option value="<?php echo $user->id; ?>" <?php if($id_u_or_group == $user->id) echo 'đã chọn'; ?>><?php echo $user->username; ?></option><?php
                    } ?>
                    </select><?php
                } else { ?>
                    <span class="assign-empty">Để chỉ định người dùng, vui lòng tạo nó.</span><?php
                } ?>
            </label><?php
            } else { ?>
            <label class="assign-group"><?php
                if (isset($templateData['groups'])) { ?>
                    <span class="f-col">Chọn nhóm</span>
                    <select name="assign[][group]"><?php
                    foreach ($templateData['groups'] as $group) { ?>
                        <option value="<?php echo $group->id; ?>" <?php if($id_u_or_group == $group->id) echo 'đã chọn'; ?>><?php echo $group->name; ?></option><?php
                    } ?>
                    </select><?php
                } else { ?>
                    <span class="assign-empty">Để chỉ định nhóm, vui lòng tạo nó.</span><?php
                } ?>
            </label><?php
            } ?>
        </div><?php
        ...
        <button type="button" class="btn btn-info add-assign-js"><i class="icon-plus"></i>Thêm</button>
    </fieldset>

    <button type="submit" class="btn btn-primary btn-large pull-right"><?php echo (true) ? 'Lưu' : 'Tạo'; ?></button>
</form>
...
<div class="condition-control-group add-assign-bl" style="display: none;">
    ...
    <button type="button" class="btn btn-danger pull-right remove-condition-js"><i class="icon-trash"></i>Xóa</button>
</div>
