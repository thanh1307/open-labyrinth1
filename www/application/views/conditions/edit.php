...
<h1 class="page-header">
    <?php echo __('Bài tập cho ').$condition->name; ?>
    <button class="btn btn-primary btn-large pull-right addConditionAssign" type="button"><?php echo __('Thêm bài tập'); ?></button>
</h1>
                ...
                <td>
                    <select name="changedConditionAssign[<?php echo $assign->id; ?>]" class="conditionAssign"><?php
                        foreach ($templateData['scenarios'] as $scenario) { ?>
                            <option value="<?php echo $scenario->id; ?>" <?php if($scenario->id == $assign->scenario_id) echo 'đã chọn'; ?>><?php
                                echo $scenario->title; ?>
                            </option><?php
                        } ?>
                    </select>
