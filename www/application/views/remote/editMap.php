...
if (isset($templateData['service'])) { ?>
    <table width="100%" bgcolor="#ffffff"><tr><td>
                <table width="100%" bgcolor="#ffffff"><tr><td>
                            <h4><?php echo __('Hệ thống'); ?></h4>
                            <p><?php echo __('tên dịch vụ'); ?>: <?php echo $templateData['service']->name; ?></p>
                            <p>service IP mask: <?php echo $templateData['service']->ip; ?></p>
                            <p><?php echo __('Hệ thống'); ?>:</p>
                            <?php if(count($templateData['service']->maps) > 0) { ?>
                            <?php foreach($templateData['service']->maps as $map) { ?>
                            <p><?php echo $map->map->name; ?> (<?php echo $map->map->id; ?>) [<a href="<?php echo URL::base(); ?>remoteServiceManager/deleteMap/<?php echo $templateData['service']->id; ?>/<?php echo $map->id; ?>"><?php echo __('xóa'); ?></a>]</p>
                            <?php } ?>
                            <?php } ?>
                            <hr>
                            <form id="form1" name="form1" method="post" action="<?php echo URL::base(); ?>remoteServiceManager/addMap/<?php echo $templateData['service']->id; ?>">
                                <table width="100%" border="0" cellspacing="0" cellpadding="4">
                                    <tr>
                                        <td><p><?php echo __('Chọn hệ thống để thêm dịch vụ'); ?></p></td>
                            ...
                            <hr>
                            <p><a href="#"><?php echo __('dịch vụ từ xa'); ?></a></p>
                        </td></tr></table>
            </td></tr></table>
<?php } ?>