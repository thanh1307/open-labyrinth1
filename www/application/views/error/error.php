...
<div id="kohana_error">
    <h2>
        ...
        <div>Browser info: <?php echo $browser_info; ?></div>
        <div>Browser version: <?php echo $browser_version; ?></div>
        <div>Client resolution: <?php echo $client_resolution; ?></div>
    </h2>
	...
	<div id="<?php echo $error_id ?>" class="content">
		...
		<?php foreach (Debug::trace($trace) as $i => $step): ?>
			<li>
				...
				<div id="<?php echo $args_id ?>" class="collapsed" style="display: none;">
					<table cellspacing="0">
				...
				<?php if (isset($source_id)): ?>
					<pre id="<?php echo $source_id ?>" class="source collapsed" style="display: none;"><code><?php echo $step['source'] ?></code></pre>
				<?php endif ?>
			</li>
	...
	<h2><a href="#<?php echo $env_id = $error_id.'environment' ?>" onclick="return koggle('<?php echo $env_id ?>')"><?php echo __('Môi trường') ?></a></h2>
	<div id="<?php echo $env_id ?>" class="content collapsed" style="display:none;">
		<?php $included = get_included_files() ?>
		<h3><a href="#<?php echo $env_id = $error_id.'environment_included' ?>" onclick="return koggle('<?php echo $env_id ?>')"><?php echo __('Các tập tin đi kèm') ?></a> (<?php echo count($included) ?>)</h3>
		...
		<h3><a href="#<?php echo $env_id = $error_id.'environment_loaded' ?>" onclick="return koggle('<?php echo $env_id ?>')"><?php echo __('Tiện ích mở rộng đã tải') ?></a> (<?php echo count($included) ?>)</h3>
		<div id="<?php echo $env_id ?>" class="collapsed">
			<table cellspacing="0">
				<?php foreach ($included as $file): ?>
				<tr>
