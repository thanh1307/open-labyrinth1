...
<div class="page-header">
    <h1><?php echo __('Quá trình kịch bản').' - "'.$templateData['webinar']->title; ?>"</h1>
</div>
...
<div class="report-type">
    <span class="report-type-title"><?php echo __('Loại báo cáo') ?></span>
    <div class="radio_extended btn-group">
        <input type="radio" name="typeReport" id="4R" checked/>
        <label class="btn" for="4R" data-class="btn-info"><?php echo __('Báo cáo 4R'); ?></label>
        <input type="radio" name="typeReport" id="SCT"/>
        <label class="btn" for="SCT" data-class="btn-info"><?php echo __('Báo cáo SCT'); ?></label>
        <input type="radio" name="typeReport" id="Poll"/>
        <label class="btn" for="Poll" data-class="btn-info"><?php echo __('Thăm dò'); ?></label>
    ...
    <div class="radio_extended btn-group" id="reportByFirstOrLastAttempt">
        ...
        <label class="btn" for="reportByLatest" data-class="btn-info"><?php echo __('Lần thử cuối cùng'); ?></label>
        <input type="radio" name="sortReport" id="reportByFirst" value="0" <?php if(!$reportByLatest) echo 'checked'; ?> />
        <label class="btn" for="reportByFirst" data-class="btn-info"><?php echo __('Lần thử đầu tiên'); ?></label>
    </div>
    ...
    <div id="discrepancyMap" class="alert alert-error hide">
        ...
        Kịch bản bạn chọn không có trong hệ thống.
    </div>
</div>
...
<?php if(isset($templateData['webinarData']) && isset($templateData['usersMap'])) { ?>
    <table class="table table-striped table-bordered" id="my-labyrinths">
        <tbody>
        <tr>
            <td style="text-align: center; font-weight: bold; background: #FFFFFF;font-style: normal; font-size: 14px" rowspan="2" colspan="2">Người dùng</td>
            <td style="text-align: center; font-weight: bold; background: #FFFFFF;font-style: normal; font-size: 14px" rowspan="2">Đưa vào báo cáo</td>
            <td style="text-align: center; font-weight: bold; background: #FFFFFF;font-style: normal; font-size: 14px; display: none;" rowspan="2" id="expert-th-js">Xuất</td>
            <?php
            $stepsHeaders = array();
                        ...
                        $stepsHeaders[$stepKey]['html']  =
                            ...
                            (isset($templateData['webinarStepMap'][$stepKey]) ?
                                ...
                                : $templateData['webinarStepMap'][$stepKey]->name.$changeStepLink : '-').(
                                    ...
                                    ? ' <a class="reportStepType" data-toggle="tooltip" data-placement="top" title="" data-original-title="Lấy báo cáo ở bước này" href="'.URL::base().'webinarManager/stepReport4R/'.$webinarId.'/'.$stepKey.'/'.$webinarId.'" style="text-decoration: none;font-size: 130%;"><i class="icon-eye-open"></i></a>
                                        <a data-toggle="tooltip" data-placement="top" title="" data-original-title="Xuất bản báo cáo 4R ở bước này" href="'.URL::base().'webinarManager/publishStep/'.$webinarId.'/'.$stepKey.'" style="text-decoration: none;font-size: 130%;"><i class="icon-upload"></i></a>'
                                    : '') . '</td>';
                    }
                }
            }
            ...
            <td style="text-align: center; font-weight: bold;"><?php
                ...
                if (isset($v['showReport']) AND $v['showReport'] AND $userType != 'learner' AND $userType != 'reviewer') { ?>
                    <a
                        ...
                        data-placement="top"
                        title=""
                        data-original-title="Lấy báo cáo cho hệ thống"
...
<div class="form-actions">
    <a class="btn btn-info pull-right" href="<?php echo URL::base().'dforumManager/viewForum/'.$templateData['webinar']->forum_id;?>">
        <i class="icon-comment icon-white"></i>Đi đến chủ đề diễn đàn
    </a>
</div>
