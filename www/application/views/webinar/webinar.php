...
<form class="form-horizontal" id="webinarForm" name="webinarForm" method="post" action="<?php echo URL::base().'webinarmanager/save'; ?>">
    <h1>
        <?php echo $isScenario ? 'Chỉnh sửa' : 'Sáng tạo'; echo __(" Kịch bản"); ?>
        <input type="submit" class="btn btn-primary btn-large submit-webinar-btn pull-right" name="submit" value="<?php echo $isScenario ? 'Lưu kịch bản' : 'Sáng tạo kịch bản'; ?>"/>
    </h1>
    <input type="hidden" name="webinarId" value="<?php if($isScenario) echo $scenario->id; ?>">
    <fieldset class="fieldset">
        <legend><?php echo __('Chi tiết kịch bản'); ?></legend>
        <div class="control-group">
            <label class="control-label" for="title"><?php echo __('Tiêu đề kịch bản'); ?></label>
            <div class="controls">
                <input type="text" class="span6" id="title" name="title" value="<?php if($isScenario) echo $scenario->title; ?>">
            </div>
        </div>
        <div class="control-group">
            <label class="control-label"><?php echo __('Chuyển đổi các bước'); ?></label>
            <div class="controls">
                <div class="radio_extended btn-group">
                    ...
                    <label for="manually" data-class="btn-info" class="btn active btn-info">Thủ công</label>
                    <input type="radio" name="switchingSteps" id="automatic" value="automatic" <?php if($changeStep == 'automatic') echo 'checked'; ?>>
                    <label for="automatic" data-class="btn-info" class="btn">Tự động</label>
                </div>
            </div>
        </div><?php
        if ( ! $isScenario) {
            ...
            <div class="control-group">
                <label class="control-label"><?php echo __('Sử dụng diễn đàn hiện có') ?></label>
                <div class="controls">
                    <div class="radio_extended btn-group"><?php
                        if ($forums) {?>
                            <input autocomplete="off" type="radio" id="use" name="use" value="1" />
                            <label data-class="btn-info" class="btn" for="use"><?php echo __('Sử dụng'); ?></label><?php
                        }?>
                        <input autocomplete="off" type="radio" id="notUse" name="use" value="0" checked="checked" />
                        <label data-class="btn-info" class="btn" for="notUse" rel="tooltip" title="Will be created a new forum"><?php echo __('Không sử dụng'); ?></label>
                    </div>
                </div>
            </div>
            <div class="control-group submitSettingsContainer hide">
                <label for="forum" class="control-label"><?php echo __('Diễn đàn') ?></label>
                <div class="controls">
                ...
                foreach($forums as $forum) {
                    ...
                    <div class="topics hide" id="topics-<?php echo $forum['id'];?>">
                        <label for="topic" class="control-label"><?php echo __('Chủ đề') ?></label>
                        <div class="controls">
                            <select id="topic" name="topic">
                                <option value="0">Hãy chọn</option><?php
            ...
            <fieldset class="fieldset step-container-<?php echo $step->id; ?>" stepId="<?php echo $step->id; ?>">
                ...
                <div id="labyrinth-container-<?php echo $step->id; ?>" containerId="<?php echo $step->id; ?>"><?php
                    if(count($step->maps)) {
                        ...
                        <div class="control-group labyrinth-item-<?php echo $index; ?>" itemNumber="<?php echo $index ?>">
                            <label for="s<?php echo $step->id; ?>-labyrinth-<?php echo $index; ?>" class="control-label">Hệ thống #<?php echo $index ?></label>
                            <div class="controls">
                                ...
                                <div class="poll-node-section">
                                    <label>
                                        ...
                                        <button type="button" class="cumulative-reset btn btn-info" style="<?php if ( ! $stepMap->cumulative) echo 'display: none;'; ?>" data-scenario="<?php echo $templateData['webinar']->id; ?>" data-map="<?php echo $mapId; ?>" data-loading-text="Đang tải...">Đặt lại bộ đếm</button>
                                    </label>
                                </div>
                                ...
                                <div class="poll-node-section"><?php
                                    ...
                                    <button type="button" class="btn btn-info poll-node-js" data-id="<?php echo $mapId; ?>" data-loading-text="Đang tải...">
                                    Thêm nút thăm dò ý kiến
                                    </button>
                                </div>
                            </div>
                        </div><?php
                ...
                <div>
                    <button class="btn btn-info add-labyrinth-btn" type="button" containerId="<?php echo $step->id; ?>">
                        <i class="icon-plus-sign"></i>Thêm bản đồ hoặc phần
                    </button>
                </div>
    ...
    <div><button class="btn btn-info add-step-btn" type="button"><i class="icon-plus-sign"></i>Thêm bước</button></div>
    ...
    <div><span class="btn btn-info add-macros-btn"><i class="icon-plus-sign"></i>Thêm Macros</span></div>
    ...
    <table id="assign-users" class="table table-bordered table-striped">
        ...
        <thead>
            <tr>
                ...
                <th><a href="javascript:void(0);">Người dùng</a></th>
            </tr>
        </thead>
        <tbody><?php
    ...
    <table id="assign-users" class="table table-bordered table-striped">
        ...
        <tr>
            <th style="text-align: center">Hành động</th>
            <th><a href="javascript:void(0);">Nhóm</a></th>
        </tr>
        </thead>
        <tbody><?php
    ...
    <div class="form-actions">
        <div class="map-error-empty" style="float: left;margin-top: 12px;color: red;display: none;">Chọn hệ thống cho từng bước</div>
        <div class=" pull-right">
            <input type="submit" class="btn btn-primary btn-large submit-webinar-btn" name="submit" value="<?php echo isset($templateData['webinar']) ? 'Lưu' : 'Sáng tạo'; ?> <?php echo __('Kịch bản'); ?>" />
        </div>
    </div>
</form>
