...
abstract class Base_DB_ORM_Field_Decimal extends DB_ORM_Field {
     ...
     * @throws Kohana_InvalidProperty_Exception       indicates that the specified value does
     *                                              not validate
     */
    public function __construct(DB_ORM_Model $model, Array $metadata = array()) {
        ...
        if ( ! ($default instanceof DB_SQL_Expression)) {
            ...
            if ( ! $this->validate($default)) {
                throw new Kohana_InvalidProperty_Exception('Message: Unable to set default value for field. Reason: Value :value failed to pass validation constraints.', array(':value' => $default));
            }
        }
                ...
                if ( ! ($value instanceof DB_SQL_Expression)) {
                    if ($value !== NULL) {
                        ...
                        if ( ! $this->validate($value)) {
                            throw new Kohana_InvalidProperty_Exception('Message: Unable to set the specified property. Reason: Value :value failed to pass validation constraints.', array(':value' => $value));
                        }
                    }
                    else if ( ! $this->metadata['nullable']) {
