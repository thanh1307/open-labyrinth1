	...
	tinymce.create('tinymce.plugins.PastePlugin', {
		init : function(ed, url) {
			...
			function grabContent(e) {
				...
				// Styles needs to be applied after the element is added to the document since WebKit will otherwise Xóa tất cả styles
		...
		_preProcess : function(pl, o) {
			...
			if (/class="?Mso|style="[^"]*\bmso-|w:WordDocument/i.test(h) || o.wordContent) {
				...
				// Xóa tất cả spans if no styles is to be retained
			...
			// Class attribute options are: leave all as-is ("none"), Xóa tất cả ("all"), or remove only those starting with mso ("mso").
		...
		_postProcess : function(pl, o) {
			...
			if (o.wordContent) {
				...
				if ((tinymce.is(styleProps, "string")) && (styleProps !== "all") && (styleProps !== "*")) {
					...
					each(dom.select('*', o.node), function(el) {
						...
						// Xóa tất cả of the existing styles
			...
			// Xóa tất cả style information or only specifically on WebKit to avoid the style bug on that browser
		...
		_insertPlainText : function(content) {
			...
			if ((typeof(content) === "string") && (content.length > 0)) {
				// If HTML content with line-breaking tags, then Xóa tất cả cr/lf chars because only tags will break a line
